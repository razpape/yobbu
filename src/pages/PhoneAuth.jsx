...
function ProfileStep({ isFr, phone, onComplete }) {
  ...
  const roles = [
    {
      id: 'traveler',
      icon: <Plane size={20} color={role === 'traveler' ? '#10B981' : '#6B7280'} />,
    ...
    {
      id: 'both',
      icon: <Users size={20} color={role === 'both' ? '#10B981' : '#6B7280'} />,
  ...
  const label = (text) => (
    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#1F2937', marginBottom: 6, fontFamily: "'DM Sans', sans-serif" }}>
      {text}
    </label>
  )
  ...
  const continueBtn = (onClick, disabled, text) => (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      background: !disabled ? '#10B981' : '#E0DAD0',
    ...
    <div style={{ display: 'flex', gap: 6, marginBottom: 28 }}>
      {[1,2,3].map(n => (
        <div key={n} style={{
          ...
          background: n <= sub ? '#10B981' : '#E0DAD0',
  ...
  if (sub === 1) return (
    <div style={{ padding: '32px 28px', flex: 1, display: 'flex', flexDirection: 'column' }}>
      {dots}
      <h2 style={{ fontSize: 22, fontWeight: 700, color: '#1F2937', marginBottom: 4, fontFamily: "'DM Sans', sans-serif" }}>
        {isFr ? 'Bienvenue ! Présentez-vous.' : 'Welcome! Tell us about yourself.'}
      </h2>
      <p style={{ fontSize: 14, color: '#6B7280', marginBottom: 24, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <p style={{ fontSize: 13, fontWeight: 700, color: '#1F2937', marginBottom: 10, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
        {roles.map(opt => (
          <button key={opt.id} onClick={() => setRole(opt.id)} style={{
            ...
            border: `2px solid ${role === opt.id ? '#10B981' : '#E0DAD0'}`,
            borderRadius: 14,
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#1F2937', marginBottom: 2 }}>{opt.title}</div>
  ...
  if (sub === 2) return (
    <div style={{ padding: '32px 28px', flex: 1, display: 'flex', flexDirection: 'column' }}>
      {dots}
      <button onClick={subBack} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 6, color: '#6B7280', fontFamily: "'DM Sans', sans-serif", fontSize: 13 }}>
      ...
      <p style={{ fontSize: 14, color: '#6B7280', marginBottom: 6, fontFamily: "'DM Sans', sans-serif" }}>
        {isFr ? 'Étape 2 sur 3' : 'Step 2 of 3'}
      </p>
      <p style={{ fontSize: 13, color: '#6B7280', marginBottom: 24, lineHeight: 1.5, fontFamily: "'DM Sans', sans-serif" }}>
        {isFr
  ...
  return (
    <div style={{ padding: '32px 28px', flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ width: 52, height: 52, background: '#D1F4E7', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#10B981" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg>
      </div>
      ...
      <div style={{ background: '#F9F7F3', borderRadius: 14, padding: '16px 18px', marginBottom: 24, border: '1px solid #E8E0D4' }}>
        <div style={{ fontSize: 13, color: '#6B7280', marginBottom: 8, fontFamily: "'DM Sans', sans-serif" }}>
          {isFr ? 'Résumé de votre compte' : 'Your account summary'}
        </div>
        <div style={{ fontSize: 15, fontWeight: 600, color: '#1F2937', fontFamily: "'DM Sans', sans-serif", marginBottom: 4 }}>
          {firstName} {lastName}
        </div>
        <div style={{ fontSize: 13, color: '#6B7280', fontFamily: "'DM Sans', sans-serif", marginBottom: 4 }}>
      ...
      {/* Terms checkbox */}
      <label style={{ display: 'flex', alignItems: 'flex-start', gap: 12, cursor: 'pointer', marginBottom: 24, padding: '14px 16px', border: `2px solid ${termsAccepted ? '#10B981' : '#E0DAD0'}`, borderRadius: 14, background: termsAccepted ? '#FDF6ED' : '#fff', transition: 'all .15s' }}>
        <input
          type="checkbox" checked={termsAccepted} onChange={e => setTerms(e.target.checked)}
          style={{ width: 18, height: 18, accentColor: '#10B981', flexShrink: 0, marginTop: 2 }}
        />
        <span style={{ fontSize: 13, color: '#1F2937', lineHeight: 1.5, fontFamily: "'DM Sans', sans-serif" }}>
          {isFr
            ? <>J'ai lu et j'accepte les <strong style={{ color: '#10B981' }}>Conditions d'utilisation</strong> et la <strong style={{ color: '#10B981' }}>Politique de confidentialité</strong> de Yobbu.</>
            : <>I have read and agree to Yobbu's <strong style={{ color: '#10B981' }}>Terms of Service</strong> and <strong style={{ color: '#10B981' }}>Privacy Policy</strong>.</>}
        </span>
      </label>
...
export default function PhoneAuth({ lang = 'en', onComplete }) {
  ...
  const backButton = (target) => (
    <button
      ...
      style={{
        ...
        borderRadius: 10, color: '#1F2937', transition: 'background .15s',
      }}
  ...
  const renderPhone = () => (
    <div style={{
      ...
      <h2 style={{
        fontSize: 24, fontWeight: 700, color: '#1F2937', marginBottom: 8,
        fontFamily: "'DM Sans', sans-serif",
      }}>
      ...
      <div style={{ marginTop: 'auto', paddingTop: 20 }}>
        <button
          ...
          style={{
            width: '100%', padding: '16px 24px',
            background: phoneValid && !loading ? '#10B981' : '#E0DAD0',
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <button
            style={{
              ...
              cursor: 'pointer', fontFamily: "'DM Sans', sans-serif",
              color: '#1F2937', transition: 'all .2s',
  ...
  const renderOtp = () => (
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: 36 }}>
        ...
        <h2 style={{
          fontSize: 22, fontWeight: 700, color: '#1F2937', marginBottom: 8,
          fontFamily: "'DM Sans', sans-serif",
        }}>
      ...
      {loading && (
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          <div style={{
            width: 22, height: 22,
            border: '3px solid #E0DAD0', borderTopColor: '#10B981',
      ...
      <div style={{ marginTop: 'auto', textAlign: 'center', paddingTop: 20 }}>
        {countdown > 0 ? (
          <p style={{ color: '#6B7280', fontSize: 13, fontFamily: "'DM Sans', sans-serif" }}>
            {isFr ? 'Renvoyer dans' : 'Resend in'}{' '}
            <span style={{ fontWeight: 700, color: '#10B981' }}>{countdown}s</span>
          </p>
        ) : (
          <button
            onClick={handlePhoneSubmit}
  ...
  return (
    <>
      <style>{`
        ...
        .auth-container {
          min-height: 100vh;
          background: linear-gradient(135deg, #10B981 0%, #A07015 25%, #7C3AED 50%, #E5A630 75%, #2563EB 100%);
          display: flex;
