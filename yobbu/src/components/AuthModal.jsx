...
export default function AuthModal({ onClose, onSuccess, lang }) {
  ...
  const s = {
    ...
    social:  (bg) => ({ width:'100%', padding:'12px', borderRadius:12, border:'1px solid rgba(0,0,0,.1)', background: bg || '#fff', fontSize:14, fontWeight:500, cursor:'pointer', fontFamily:'DM Sans, sans-serif', display:'flex', alignItems:'center', justifyContent:'center', gap:10, color:'#1A1710', marginBottom:10, transition:'all .2s', opacity: googleLoading ? .6 : 1 }),
    ...
    <div onClick={onClose} style={s.overlay}>
      <div onClick={e => e.stopPropagation()} style={s.modal}>
        ...
        {step === 'entry' && (
          <>
            {/* Google — WORKING */}
            <button style={s.social()} onClick={handleGoogle} disabled={googleLoading}>
              {googleLoading ? (
