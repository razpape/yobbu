...
const T = {
  en: {
    ...
    edit: '✏️ Edit', delete: '🗑 Delete', postNew: '+ Post a new trip',
  ...
  fr: {
    ...
    edit: '✏️ Modifier', delete: '🗑 Supprimer', postNew: '+ Poster un nouveau voyage',
      ...
      <div style={{ padding:'8px 12px' }}>
        {FLIGHTS.map((f, i) => (
          <div key={i} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 0', borderBottom: i < FLIGHTS.length - 1 ? '1px solid rgba(0,0,0,.04)' : 'none' }}>
            <div style={{ width:26, height:26, borderRadius:7, background:'#F7F3ED', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, flexShrink:0 }}>✈</div>
            <div style={{ flex:1 }}>
...
export default function ProfilePage({ user, lang: initialLang, onSignOut, setView }) {
  ...
  const menuItems = [
    { key:'trips', icon:'✈', label: t.menuTrips },
    { key:'settings', icon:'⚙', label: t.menuSettings },
    { key:'notifications', icon:'🔔', label: t.menuNotif, badge: notifications.length },
    { key:'help', icon:'❓', label: t.menuHelp },
  ]
          ...
          <div style={s.card}>
            {menuItems.map(({ key, icon, label, badge }) => (
              <div key={key} onClick={() => setSection(key)}
                ...
                <div style={{ width:28, height:28, borderRadius:8, background: section === key ? '#FFF8EB' : '#F7F3ED', display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, flexShrink:0 }}>
                  {icon}
                </div>
              ...
              {!loading && trips.map(trip => (
                <div key={trip.id} style={{ background:'#FDFBF7', border:'1px solid rgba(0,0,0,.06)', borderRadius:12, padding:'14px 16px', marginBottom:10 }}>
                  ...
                  <div style={{ display:'flex', gap:6 }}>
                    <button onClick={() => setEditingTrip(trip)} style={{ fontSize:11, fontWeight:600, padding:'4px 12px', borderRadius:7, border:'1px solid rgba(0,0,0,.1)', background:'#fff', color:'#3D3829', cursor:'pointer', fontFamily:'DM Sans, sans-serif' }}>
                      {t.edit}
                    </button>
                    <button onClick={() => deleteTrip(trip.id)} style={{ fontSize:11, fontWeight:600, padding:'4px 12px', borderRadius:7, border:'1px solid #FECACA', background:'#FEF2F2', color:'#DC2626', cursor:'pointer', fontFamily:'DM Sans, sans-serif' }}>
                      {t.delete}
                    </button>
                  </div>
                </div>
            ...
            <div style={s.section}>
              ...
              {!loading && notifications.map(({ type, trip, id }) => (
                <div key={id} style={{
                  ...
                  <div style={{ fontSize:24, flexShrink:0 }}>{type === 'approved' ? '✅' : '⚠️'}</div>
                  <div style={{ flex:1 }}>
